...
--Daría igual poner en este caso el DISTINCT o no, al NOT IN no le importa si se repiten los valores.
SELECT *
FROM EI.ALUMNO AL
WHERE AL.NAL NOT IN(SELECT DISTINCT MAT.ALUM FROM EI.MATRICULA MAT WHERE AÑO = 2001)
AND AL.NAL IN (SELECT DISTINCT MAT2.ALUM FROM EI.MATRICULA MAT2 WHERE AÑO IN (2000,2002));
----------------------------Dpm. En vez de IN(2000,2002), también podría haber puesto AÑO = 2000 OR AÑO = 2002.
SELECT AL.NOMBRE
FROM EI.ALUMNO AL
...
SELECT *
FROM EI.PROFESOR PROF
WHERE PROF.DESPACHO NOT IN(SELECT PROF2.DESPACHO FROM EI.PROFESOR PROF2 WHERE PROF.DESPACHO = PROF2.DESPACHO AND PROF.DNI <> PROF2.DNI);
---------------------------------Dpm, perfecto.
SELECT *
FROM EI.PROFESOR PROF
...
FROM MF.TELEFONO TEL 
WHERE TEL.NUMERO IN (SELECT LLAM.TF_ORIGEN FROM MF.LLAMADA LLAM WHERE TEL.NUMERO = LLAM.TF_ORIGEN AND TO_CHAR(LLAM.FECHA_HORA,'DD/MM/YYYY')='16/10/2006')
);
-------------------------------------Perfecto. || es como << en C++.
SELECT CLI.NOMBRE
FROM MF.CLIENTE CLI
WHERE CLI.DNI IN (SELECT TEL.CLIENTE
FROM MF.TELEFONO TEL
...
JOIN MF.TELEFONO TEL
ON CLI.DNI = TEL.CLIENTE
WHERE TEL.NUMERO IN (SELECT LLAM.TF_ORIGEN FROM MF.LLAMADA LLAM WHERE LLAM.DURACION < 60*1.5);
--------------------------Perfectísimo.
SELECT CLI.NOMBRE
FROM MF.CLIENTE CLI
JOIN
...
ON LLAM.TF_ORIGEN = TEL.NUMERO
JOIN MF.COMPAÑIA C
ON TEL.COMPAÑIA= C.CIF
JOIN MF.CLIENTE CLI
ON TEL.CLIENTE = CLI.DNI
WHERE C.NOMBRE = 'Kietostar' AND LLAM.TF_ORIGEN NOT IN (SELECT TF_ORIGEN FROM MF.LLAMADA LLAM2 WHERE EXTRACT(MONTH FROM LLAM2.FECHA_HORA) = 9);
--------------------------------Aunque no me salga Antonio está más o menos bien. 
...
FROM MF.LLAMADA LLAM
WHERE ((SELECT COUNT(*) FROM MF.LLAMADA LLAM2 WHERE LLAM.TF_ORIGEN = LLAM2.TF_ORIGEN AND TF_DESTINO = '666789789') = 0)
AND ((SELECT COUNT(*) FROM MF.LLAMADA LLAM3 WHERE LLAM.TF_ORIGEN = LLAM3.TF_ORIGEN AND TF_DESTINO = '654234234')<>0);
--------------------------------------Perfecto, también lo he hecho de otra manera.
SELECT *
FROM MF.TELEFONO TEL
WHERE TEL.NUMERO IN
(SELECT TF_ORIGEN
...
SELECT COUNT(DISTINCT PROF.DESPACHO) AS N_DESPACHOS
FROM EI.PROFESOR PROF;
--------------------------------------------------------
...
WHERE (SELECT COUNT(*) FROM EI.RECOMENDACIONES RECOM WHERE ASIG.IDASIG = RECOM.IDASIG1) >= 2;
--------------------------------------------------------------------------------------------Lo he hecho de dos formas, guay.
...
SELECT * 
FROM EI.PROFESOR PROF
WHERE (SELECT COUNT(*) FROM EI.PROFESOR PROF2 WHERE PROF.ANT < PROF2.ANT) >= 5;
------------------------------------------------------------------------------Es difícil, viene bien practicar esta subconsulta.
SELECT PROF.NOMBRE
FROM EI.PROFESOR PROF
...
SELECT AL.NOMBRE,COUNT(*),AVG(MAT.FEB_JUN)
FROM EI.MATRICULA MAT
JOIN EI.ALUMNO AL
ON MAT.ALUM = AL.NAL
WHERE AÑO=2002
GROUP BY AL.NOMBRE
HAVING AVG(MAT.FEB_JUN) >= 5;--Los join siempre van antes que los where.
-------------------------------------------------------------------------
...
SELECT ASIG.NOMBRE,COUNT(*) 
FROM EI.MATRICULA MAT
JOIN EI.ASIGNATURA ASIG
ON MAT.IDASIG = ASIG.IDASIG
WHERE MAT.AÑO = 2002
GROUP BY ASIG.NOMBRE
HAVING COUNT(*) = (--Pongo la misma consulta pero le retorno solo maxcount para que compare. Podria poner loq ue quiera en el having, también números directamente.
    SELECT MAX(COUNT(*))
    FROM EI.MATRICULA MAT
    JOIN EI.ASIGNATURA ASIG
    ON MAT.IDASIG = ASIG.IDASIG
...
CONSTRAINT cursosNomUnico UNIQUE (nomCurso));--No funciona pero es para ver la estructura.
...
ALTER TABLE ASIGNATURA ADD CONSTRAINT cursos CHECK (curso IN (’1’,’2’,’3’,’4’,’5’));

--Insertado
INSERT INTO ORDENADOR
VALUES ('Ord220', 'Ordenador Multimedia', 'Aula 8');

--Borra de la tabla ORDENADOR todos los ordenadores que sean estaciones SUN.
DELETE FROM ORDENADOR 
WHERE tipo = 'Estación Sun';

--Modifica el cuatrimestre y la especialidad de la asignatura 'Análisis Numérico'.
UPDATE ASIGNATURA 
SET cuat = 2, esp = 'S' 
...
ALTER TABLE LIBRO
ADD CONSTRAINT nombreNoNulo
    CHECK(NOMBRE IS NOT NULL);

ALTER TABLE LIBRO
ADD CONSTRAINT antiguedad
    CHECK(ANTIG BETWEEN 0 AND 500); --No hay libros mas antiguos, entre muchísimas comillas Xd.
    
ALTER TABLE LIBRO
DROP CONSTRAINT antiguedad;
...
--Sin constraint
ALTER TABLE tabla MODIFY ( columna NOT NULL );	
--BETWEEN
ALTER TABLE tabla CHECK ( columna BETWEEN valor1 AND valor2 )